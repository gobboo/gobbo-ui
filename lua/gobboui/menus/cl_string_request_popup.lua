...
function PANEL:AddOption(name, callback)
    local btn = vgui.Create("GOBBO.TextButton", self.ButtonHolder)
    btn:SetText(name)
...
function PANEL:LayoutContent(w, h)
    ...
    self.TextEntry:SetTall(GOBBO.Scale(32))
    self.TextEntry:Dock(TOP)
    self.TextEntry:DockMargin(0, 0, 0, GOBBO.Scale(10))

    for k,v in ipairs(self.Buttons) do
        v:SizeToText()
        v:Dock(LEFT)
        v:DockMargin(GOBBO.Scale(4), 0, GOBBO.Scale(4), 0)
    end
...
vgui.Register("GOBBO.StringRequest", PANEL, "GOBBO.Frame")

GOBBO.UI.Overrides.Derma_StringRequest = GOBBO.UI.Overrides.Derma_StringRequest or Derma_StringRequest

Derma_StringRequest = GOBBO.UI.CreateToggleableOverride(GOBBO.UI.Overrides.Derma_StringRequest, function(title, text, placeholderText, enterCallback, cancelCallback, buttonText, cancelText)
    cancelCallback = cancelCallback or function() end
    buttonText = buttonText or "OK"
    cancelText = cancelText or "Cancel"

    local msg = vgui.Create("GOBBO.StringRequest")
    msg:SetTitle(title)
    msg:SetText(text)
