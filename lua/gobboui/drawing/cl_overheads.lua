...
GOBBO.RegisterFontUnscaled("UI.Overhead", "Open Sans Bold", 100)
...
local function drawOverhead(ent, pos, text, ang, scale)
    ...
    GOBBO.SetFont("UI.Overhead")
    local w, h = GOBBO.GetTextSize(text)
    w = w + 40
    h = h + 6
    ...
    if not Icon then
        GOBBO.DrawRoundedBox(12, x, y, w, h, GOBBO.Colors.Primary)
        GOBBO.DrawText(text, "UI.Overhead", 0, y + 1, GOBBO.Colors.PrimaryText, TEXT_ALIGN_CENTER)
    else
        x = x - 40
        GOBBO.DrawRoundedBox(12, x, y, h, h, GOBBO.Colors.Primary)
        GOBBO.DrawRoundedBoxEx(12, x + (h - 12), y + h - 20, w + 15, 20, GOBBO.Colors.Primary, false, false, false, true)
        GOBBO.DrawText(text, "UI.Overhead", x + h + 15, y + 8, GOBBO.Colors.PrimaryText)
        GOBBO.DrawImage(x + 10, y + 10, h - 20, h - 20, Icon, color_white)
    end
    end3d2d()
...
local entOffset = 2
function GOBBO.DrawEntOverhead(ent, text, angleOverride, posOverride, scaleOverride)
...
function GOBBO.DrawNPCOverhead(ent, text, angleOverride, offsetOverride, scaleOverride)
    if checkDistance(ent) then return end
...
function GOBBO.EnableIconOverheads(new)
    local oldIcon = Icon
