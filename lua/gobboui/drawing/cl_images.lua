...
do
    ...
    function GOBBO.DrawProgressWheel(x, y, w, h, col)
        local progSize = min(w, h)
        setMaterial(progressMat)
        setDrawColor(col.r, col.g, col.b, col.a)
        drawTexturedRectRotated(x + w * .5, y + h * .5, progSize, progSize, -curTime() * 100)
    end
    drawProgressWheel = GOBBO.DrawProgressWheel
end

local materials = {}
local grabbingMaterials = {}

local getImage = GOBBO.GetImage
getImage(GOBBO.ProgressImageURL, function(mat)
    progressMat = mat
end)

local drawTexturedRect = surface.DrawTexturedRect
function GOBBO.DrawImage(x, y, w, h, url, col)
...
local drawTexturedRectRotated = surface.DrawTexturedRectRotated
function GOBBO.DrawImageRotated(x, y, w, h, rot, url, col)
    if not materials[url] then
...
function GOBBO.DrawImgur(x, y, w, h, imgurId, col)
    local url = "https://i.imgur.com/" .. imgurId .. ".png"
    GOBBO.DrawImage(x, y, w, h, url, col)
end

function GOBBO.DrawImgurRotated(x, y, w, h, rot, imgurId, col)
    local url = "https://i.imgur.com/" .. imgurId .. ".png"
    GOBBO.DrawImageRotated(x, y, w, h, rot, url, col)
end