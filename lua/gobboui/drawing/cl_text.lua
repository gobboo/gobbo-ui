...
local setFont = GOBBO.SetFont
local getTextSize = GOBBO.GetTextSize
local setTextPos = surface.SetTextPos
local setTextColor = surface.SetTextColor
local drawText = surface.DrawText

function GOBBO.DrawSimpleText(text, font, x, y, col, xAlign, yAlign)
    setFont(font)
...
local drawSimpleText = GOBBO.DrawSimpleText
local gmatch = string.gmatch
local find = string.find
local max = math.max
local select = select

function GOBBO.DrawText(text, font, x, y, col, xAlign, yAlign)
    local curX = x
    local curY = y
...
function GOBBO.DrawShadowText(text, font, x, y, col, xAlign, yAlign, depth, shadow)
    shadow = shadow or 50
...
local drawShadowText = GOBBO.DrawShadowText

function GOBBO.DrawDualText(title, subtitle, x, y, h)
    x = x or 0
    y = y or 0
...
function GOBBO.WrapText(text, width, font) --Edit of https://github.com/FPtje/DarkRP/blob/master/gamemode/modules/base/cl_util.lua#L21
...
function GOBBO.EllipsesText(text, width, font)
    local chachedName = text .. width .. font
