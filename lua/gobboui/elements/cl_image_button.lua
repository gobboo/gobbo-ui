...
--- @class GOBBO.ImageButton : GOBBO.Button
local PANEL = {}
...
function PANEL:Init()
    self.ImageCol = GOBBO.CopyColor(color_white)
    self:SetImageURL(GOBBO.ProgressImageURL)
...
function PANEL:Paint(w, h)
    ...
    if not self:IsEnabled() then
        GOBBO.DrawImage(imageOffset, imageOffset, imageSize, imageSize, self:GetImageURL(), self:GetDisabledColor())
        return
    end
    ...
    self.ImageCol = GOBBO.LerpColor(FrameTime() * 12, self.ImageCol, col)

    GOBBO.DrawImage(imageOffset, imageOffset, imageSize, imageSize, self:GetImageURL(), self.ImageCol)
end

vgui.Register("GOBBO.ImageButton", PANEL, "GOBBO.Button")