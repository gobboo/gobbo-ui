...
function PANEL:Paint(w, h)
    if not self:IsEnabled() then
        ...
        GOBBO.DrawSimpleText("Disabled", "UI.TextEntry", GOBBO.Scale(4), h / 2, GOBBO.Colors.SecondaryText, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        return
    end

    if self:GetValue() == "" then
        GOBBO.DrawSimpleText(self:GetPlaceholderText() or "", "UI.TextEntry", GOBBO.Scale(10), h / 2, self.PlaceholderTextCol, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end
