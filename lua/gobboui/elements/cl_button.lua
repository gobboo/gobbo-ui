...
function PANEL:Init()
    ...
    local btnSize = GOBBO.Scale(30)
    self:SetSize(btnSize, btnSize)

    self.NormalCol = GOBBO.CopyColor(GOBBO.Colors.Primary)
    self.HoverCol = GOBBO.OffsetColor(self.NormalCol, -15)
    self.ClickedCol = GOBBO.OffsetColor(self.NormalCol, 15)
    self.DisabledCol = GOBBO.CopyColor(GOBBO.Colors.Disabled)
...
function PANEL:Paint(w, h)
    ...
    self.BackgroundCol = GOBBO.LerpColor(FrameTime() * 12, self.BackgroundCol, bgCol)
...
vgui.Register("GOBBO.Button", PANEL, "Panel")