...
--- @class GOBBO.NavbarItem : GOBBO.Button
local PANEL = {}

AccessorFunc(PANEL, "Name", "Name", FORCE_STRING)
AccessorFunc(PANEL, "Color", "Color")

GOBBO.RegisterFont("UI.NavbarItem", "Open Sans SemiBold", 22)

function PANEL:Init()
    self:SetName("N/A")
    self:SetColor(GOBBO.Colors.Primary)

    self.NormalCol = GOBBO.Colors.PrimaryText
    self.HoverCol = GOBBO.Colors.SecondaryText

    self.TextCol = GOBBO.CopyColor(self.NormalCol)
end

function PANEL:GetItemSize()
    GOBBO.SetFont("UI.NavbarItem")
    return GOBBO.GetTextSize(self:GetName())
end
...
function PANEL:Paint(w, h)
    ...
    GOBBO.DrawSimpleText(self:GetName(), "UI.NavbarItem", w / 2, h / 2, self.TextCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end

vgui.Register("GOBBO.NavbarItem", PANEL, "GOBBO.Button")

--- @class GOBBO.Navbar : Panel
PANEL = {}
...
function PANEL:Init()
    ...
    self.BackgroundCol = GOBBO.OffsetColor(GOBBO.Colors.Background, 10)
end

function PANEL:AddItem(id, name, doClick, order, color)
    local btn = vgui.Create("GOBBO.NavbarItem", self)

    btn:SetName(name:upper())
    btn:SetZPos(order or table.Count(self.Items) + 1)
    btn:SetColor((IsColor(color) and color) or GOBBO.Colors.Primary)
...
function PANEL:PerformLayout(w, h)
    for k,v in pairs(self.Items) do
        v:Dock(LEFT)
        v:SetWide(v:GetItemSize() + GOBBO.Scale(30))
    end
end
...
function PANEL:Paint(w, h)
    ...
    if not self.SelectedItem then
        ...
        self.SelectionColor = GOBBO.LerpColor(FrameTime() * 10, self.SelectionColor, GOBBO.Colors.Primary)
        return
    end
    ...
    self.SelectionColor = GOBBO.LerpColor(FrameTime() * 10, self.SelectionColor, selectedItem:GetColor())

    local selectorH = GOBBO.Scale(6)
    surface.SetDrawColor(self.SelectionColor)
    surface.DrawRect(self.SelectionX + 2, h - selectorH, self.SelectionW - 4, selectorH / 2)
end

vgui.Register("GOBBO.Navbar", PANEL, "Panel")