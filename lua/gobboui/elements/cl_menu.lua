...
function PANEL:Init()
    ...
    self:SetMinimumWidth(GOBBO.Scale(100))
...
function PANEL:AddOption(strText, funcFunction)
    local pnl = vgui.Create("GOBBO.MenuOption", self)
    pnl:SetMenu(self)
    pnl:SetText(strText)
...
function PANEL:AddCVar(strText, convar, on, off, funcFunction)
    local pnl = vgui.Create("GOBBO.MenuOptionCVar", self)
    pnl:SetMenu(self)
    pnl:SetText(strText)
    if funcFunction then pnl.DoClick = funcFunction end
...
function PANEL:AddSpacer(text, func)
    ...
    local spacerCol = GOBBO.OffsetColor(GOBBO.Colors.Background, 6)
    pnl.Paint = function(p, w, h)
        surface.SetDrawColor(spacerCol)
        surface.DrawRect(0, 0, w, h)
    end

    pnl:SetTall(GOBBO.Scale(3))
    self:AddPanel(pnl)
...
vgui.Register("GOBBO.Menu", PANEL, "GOBBO.ScrollPanel")