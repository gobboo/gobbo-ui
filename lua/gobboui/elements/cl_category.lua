...
--- @class GOBBO.CategoryHeader : GOBBO.Button
local PANEL = {}

AccessorFunc(PANEL, "Title", "Title", FORCE_STRING)

GOBBO.RegisterFont("UI.CategoryHeader", "Open Sans Bold", 19)

function PANEL:Init()
    self.ArrowRotation = 0

    self.BackgroundCol = GOBBO.OffsetColor(GOBBO.Colors.Background, 6)
end
...
function PANEL:Paint(w, h)
    ...
    GOBBO.DrawSimpleText(self.Title, "UI.CategoryHeader", GOBBO.Scale(10), h / 2, GOBBO.Colors.PrimaryText, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

    self.ArrowRotation = lerp(FrameTime() * 10, self.ArrowRotation, self:GetParent():GetExpanded() and 0 or 90)

    local arrowSize = h * .45
    GOBBO.DrawImageRotated(w - h * .3 - GOBBO.Scale(4), h / 2, arrowSize, arrowSize, self.ArrowRotation, "https://pixel-cdn.lythium.dev/i/5r7ovslav", GOBBO.Colors.PrimaryText)
end

vgui.Register("GOBBO.CategoryHeader", PANEL, "GOBBO.Button")

--- @class GOBBO.Category : Panel
PANEL = {}
...
function PANEL:Init()
    self.Header = vgui.Create("GOBBO.CategoryHeader", self)

    self:SetTitle("GOBBO Category")
...
function PANEL:PerformLayout(w, h)
    ...
    self.Header:SetTall(GOBBO.Scale(26))
...
vgui.Register("GOBBO.Category", PANEL, "Panel")
