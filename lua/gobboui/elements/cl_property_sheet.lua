...
GOBBO.RegisterFont("UI.Tab", "Open Sans Bold", 16)

function PANEL:Init()
	self.BackgroundCol = GOBBO.OffsetColor(GOBBO.Colors.Background, -4)
	self.SelectedCol = GOBBO.Colors.Primary
	self.UnselectedTextCol = GOBBO.Colors.SecondaryText
	self.SelectedTextCol = GOBBO.Colors.PrimaryText

	self.Color = GOBBO.CopyColor(self.BackgroundCol)
	self.TextColor = GOBBO.CopyColor(self.UnselectedTextCol)
end
...
function PANEL:Setup(text, propertySheet, panel)
	...
	GOBBO.SetFont("UI.Tab")
	self:SetWide(GOBBO.GetTextSize(text) + GOBBO.Scale(16))
end
...
function PANEL:DoRightClick()
	...
	local tabs = vgui.Create("GOBBO.Menu", self)
...
function PANEL:Paint(w, h)
	self.Color = GOBBO.LerpColor(FrameTime() * 12, self.Color, (self:IsActive() or self:IsHovered()) and self.SelectedCol or self.BackgroundCol)
	self.TextColor = GOBBO.LerpColor(FrameTime() * 12, self.TextColor, (self:IsActive() or self:IsHovered()) and self.SelectedTextCol or self.UnselectedTextCol)

	GOBBO.DrawRoundedBoxEx(GOBBO.Scale(6), 0, 0, w, h, self.Color, true, true)
	GOBBO.DrawSimpleText(self:GetText(), "UI.Tab", w * .5, h * .5, self.TextColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end

vgui.Register("GOBBO.Tab", PANEL, "GOBBO.Button")

--- @class GOBBO.PropertySheet : Panel
PANEL = {}
...
function PANEL:Init()
	...
	self.tabScroller:SetOverlap(GOBBO.Scale(5))
	self.tabScroller:Dock(TOP)
	self.tabScroller:DockMargin(GOBBO.Scale(3), 0, GOBBO.Scale(3), 0)

	self:SetFadeTime(0.1)
	self:SetPadding(GOBBO.Scale(8))

	self.animFade = Derma_Anim("Fade", self, self.CrossFade)

	self.Items = {}

	self.BackgroundCol = GOBBO.OffsetColor(GOBBO.Colors.Background, 2)
end

function PANEL:AddSheet(label, panel, material, noStretchX, noStretchY, tooltip)
	if not IsValid(panel) then
		ErrorNoHalt("GOBBO.PropertySheet:AddSheet tried to add invalid panel!")
		debug.Trace()
		return
	end
	...
	sheet.Tab = vgui.Create("GOBBO.Tab", self)
	...
	sheet.Panel:SetPos(self:GetPadding(), GOBBO.Scale(24) + self:GetPadding())
...
function PANEL:Paint(w, h)
	...
	GOBBO.DrawRoundedBox(GOBBO.Scale(4), 0, offset, w, h - offset, self.BackgroundCol)
end

vgui.Register("GOBBO.PropertySheet", PANEL, "Panel")